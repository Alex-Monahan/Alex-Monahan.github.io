...
import { Chunked, Vector, VectorBuilderOptions, VectorBuilderOptionsAsync } from './vector/index';
declare type VectorMap = {
    [key: string]: Vector | Exclude<TypedArray, Uint8ClampedArray>;
};
...
export declare class Table<T extends {
    [key: string]: DataType;
} = any> extends Chunked<Struct<T>> implements Clonable<Table<T>>, Sliceable<Table<T>>, Applicative<Struct<T>, Table<T>> {
    /** @nocollapse */
    ...
    static new<T extends VectorMap = any>(children: T): Table<{
        [P in keyof T]: T[P] extends Vector ? T[P]['type'] : T[P] extends Exclude<TypedArray, Uint8ClampedArray> ? TypedArrayDataType<T[P]> : never;
    }>;
    static new<T extends {
        [key: string]: DataType;
